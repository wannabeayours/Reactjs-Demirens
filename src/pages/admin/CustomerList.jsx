...
function AdminGuestProfile() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      formData.append('method', 'getOnlineCustomers')
      const res = await axios.post(url, formData)
      const data = Array.isArray(res.data) ? res.data : (res.data !== 0 ? res.data : [])
      const onlineOnly = (Array.isArray(data) ? data : []).filter((c) => (
        c && (
          c.customers_online_username ||
          c.customers_online_email ||
          c.customers_online_phone ||
          typeof c.customers_online_authentication_status !== 'undefined'
        )
      ))
      setCustomers(onlineOnly)
    } catch (err) {
